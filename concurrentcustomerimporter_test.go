...
func TestMakeSortedArray(t *testing.T) {
	...
	if !flag {
		t.Error("makeSortedArray functions does not sort map")
	}
}
...
func TestConcurrency(t *testing.T) {
	...
	actualString := <-pool.Result

	if expectedString != actualString {
		t.Error("Given string was not processed correctly through the pipeline")
	}
}
...
func TestCountDomains(t *testing.T) {
	...
	actualDomainMap := countDomains(pool.Result)
